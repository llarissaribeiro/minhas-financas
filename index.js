...
function createTransactionContainer(id){
  //criar uma div para agrupar tudo que estiver nas transações
...
function createTransactionAmount(amount){
  //formatar os valores por região (país)
  ...
  if(amount > 0){
    span.textContent = `${formattedAmount} C` //crédito
    span.classList.add('transaction-amount', 'credit')
  } else{
    span.textContent = `${formattedAmount} D` //débito
...
function renderTransaction(transaction){
  ...
  document.querySelector('#transactions').append(container) //pega as seções que estão em transactions e acrescenta no container
}
...
function updateBalance(){
  //pega as transações e calcula o saldo total
